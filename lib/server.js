...
app.get('/version', (req, res) => {
    res.status(200).json({
        version: '0.9.9',  // Update this value when releasing new versions
        minVersion: '0.9.9',
...
app.post('/:index/_bulk', express.raw({ limit: '100mb', type: '*/*' }), async (req, res) => {
    try {
...
app.post('/proxy/:index/_bulk', express.raw({ limit: '100mb', type: '*/*' }), async (req, res) => {
    try {
        const targetPath = `${req.params.index}/_bulk`;
        await handleSearchOrBulk(req, res, targetPath);
    } catch (err) {
