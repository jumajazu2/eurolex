...
// TEMPORARY: set to false to stop logging search terms in the request log
const LOG_SEARCH_TERMS = true;
...
app.get('/reset-rate-limit', async (req, res) => {
    ...
    console.log('All in-memory rate limit counters have been reset.');
    
    // Also clear OpenSearch quota data for today
    try {
        ...
        const result = await opensearchRequest('POST', `/${QUOTA_INDEX}/_delete_by_query`, deleteQuery);
        
        if (result.statusCode === 200) {
            console.log(`✓ Cleared OpenSearch quota data for ${today}`);
            res.status(200).send('All rate limit counters (in-memory and OpenSearch) have been reset.');
        } else {
            console.error(`✗ Failed to clear OpenSearch quota data: ${result.statusCode}`);
            res.status(200).send('In-memory counters reset. Warning: OpenSearch cleanup failed.');
        }
    } catch (err) {
        console.error(`✗ Error clearing OpenSearch quota data: ${err.message}`);
        res.status(200).send('In-memory counters reset. Warning: OpenSearch cleanup failed.');
    }
});
// 5. Rate Limiting Middleware
// Store request counts by user identifier (not IP).
// Key formats:
//   - Trial users: "trial:{email}" and "trial-ip:{ip}" (track both email and IP)
//   - Paid users: "paid:{email}" or "paid-key:{apiKey}" (track by user, not IP)
...
// ======================================================================
// OPENSEARCH QUOTA PERSISTENCE
// ======================================================================
// This section implements persistent quota tracking using OpenSearch.
// The quota data is stored in-memory (ipRequestCounts Map) for fast
// access during request handling, but also synced to OpenSearch for
// persistence across server restarts.
//
// Index: .quota_usage_v1 (hidden index, auto-created)
// Document structure:
// {
//   "id": "2026-02-24_192.168.1.100" (or "2026-02-24_trial:user@example.com"),
//   "date": "2026-02-24",
//   "key": "192.168.1.100" (IP or "trial:email"),
//   "count": 45,
//   "email": "user@example.com" (optional),
//   "clientType": "app" | "web" | "trados" | "unknown",
//   "lastUpdated": "2026-02-24T14:30:00Z"
// }
// ======================================================================

/**
 * Detects client type based on request headers and characteristics.
 * @param {object} req - Express request object
 * @returns {string} - Client type: 'app', 'web', 'trados', or 'unknown'
 */
function detectClientType(req) {
    // Check for device ID header (Flutter app)
    if (req.headers['x-device-id']) {
        return 'app';
    }
    
    // Check for client context header (web interface)
    const clientContext = req.headers['x-client-context'];
    if (clientContext) {
        try {
            const context = JSON.parse(clientContext);
            if (context.client === 'web') {
                return 'web';
            }
            if (context.client === 'trados') {
                return 'trados';
            }
        } catch (e) {
    ...
    // Check User-Agent for Trados patterns
    const userAgent = req.headers['user-agent'] || '';
    if (userAgent.includes('Trados') || userAgent.includes('SDL')) {
        return 'trados';
    }
...
const QUOTA_INDEX = '.quota_usage_v1';

/**
 * Makes a direct HTTP request to OpenSearch.
 * Uses same credentials as the proxy (localhost:9200, admin:admin).
 * @param {string} method - HTTP method (GET, POST, PUT, DELETE)
 * @param {string} path - OpenSearch path (e.g., '/.quota_usage_v1/_doc/123')
...
async function opensearchRequest(method, path, body = null) {
    return new Promise((resolve, reject) => {
        ...
        // For bulk API, body is already NDJSON string; for other APIs, it's an object to stringify
        const postData = body ? Buffer.from(typeof body === 'string' ? body : JSON.stringify(body)) : null;
...
/**
 * Ensures the quota index exists with proper mapping.
 * Called once at startup.
 */
async function initializeQuotaIndex() {
    try {
        // Check if index exists
        const checkResult = await opensearchRequest('HEAD', `/${QUOTA_INDEX}`);
        ...
        if (checkResult.statusCode === 404) {
            ...
            await opensearchRequest('PUT', `/${QUOTA_INDEX}`, mapping);
            console.log(`✓ Created OpenSearch quota index: ${QUOTA_INDEX}`);
        } else {
            console.log(`✓ OpenSearch quota index exists: ${QUOTA_INDEX}`);
            
            // Update mapping to ensure clientType field exists (safe operation - only adds new fields)
            try {
                const mappingUpdate = {
                    properties: {
                        clientType: { type: 'keyword' }
                ...
                await opensearchRequest('PUT', `/${QUOTA_INDEX}/_mapping`, mappingUpdate);
                console.log(`✓ Updated quota index mapping with clientType field`);
            } catch (mappingErr) {
                console.log(`  Note: Could not update mapping (may already be correct): ${mappingErr.message}`);
            }
        }
    } catch (err) {
        console.error(`✗ Failed to initialize quota index: ${err.message}`);
        console.error('  Quota persistence will be disabled, but in-memory tracking continues.');
    }
}

/**
 * Loads today's quota data from OpenSearch into ipRequestCounts Map.
 * Called once at server startup to restore quota state.
 */
async function loadQuotaFromOpenSearch() {
    try {
        const today = new Date().toISOString().slice(0, 10);
        
        // Search for all documents with today's date
        const searchBody = {
            query: {
                term: { date: today }
            },
            size: 10000 // Adjust if you have more than 10k unique IPs/emails per day
        };
        
        const result = await opensearchRequest('POST', `/${QUOTA_INDEX}/_search`, searchBody);
        
        if (result.statusCode === 200 && result.body.hits && result.body.hits.hits) {
            const hits = result.body.hits.hits;
            let loadedCount = 0;
            
            for (const hit of hits) {
                const doc = hit._source;
                if (doc.key && typeof doc.count === 'number') {
                    ipRequestCounts.set(doc.key, {
                        count: doc.count,
                        date: doc.date,
                        email: doc.email || undefined,
                        clientType: doc.clientType || 'unknown'
                    });
                    loadedCount++;
                }
            }
            
            console.log(`✓ Loaded ${loadedCount} quota records from OpenSearch for ${today}`);
        }
    } catch (err) {
        console.error(`✗ Failed to load quota from OpenSearch: ${err.message}`);
        console.error('  Starting with empty quota counts. Data will still be synced.');
    }
}

/**
 * Syncs current in-memory quota data to OpenSearch.
 * Uses bulk API for efficiency.
 * Called periodically (every 60 seconds) by background task.
 */
async function syncQuotaToOpenSearch() {
    try {
        if (ipRequestCounts.size === 0) {
        ...
        for (const [key, data] of ipRequestCounts.entries()) {
            ...
            // Action line: index with document ID
            bulkLines.push(JSON.stringify({ index: { _index: QUOTA_INDEX, _id: docId } }));
            // Document line
            bulkLines.push(JSON.stringify(doc));
        }
        
        if (bulkLines.length === 0) {
            return; // No today's data to sync
        }
        
        const ndjson = bulkLines.join('\n') + '\n';
        const result = await opensearchRequest('POST', '/_bulk', ndjson);
        
        if (result.statusCode === 200 || result.statusCode === 201) {
            const recordCount = bulkLines.length / 2;
            console.log(`✓ Synced ${recordCount} quota records to OpenSearch`);
        } else {
            console.error(`✗ Quota sync failed with status ${result.statusCode}`);
            if (result.body) {
                console.error('  Response:', JSON.stringify(result.body, null, 2));
            }
        }
    } catch (err) {
        console.error(`✗ Failed to sync quota to OpenSearch: ${err.message}`);
        // Don't crash the server - in-memory tracking continues
    }
}

// Initialize quota persistence at startup
(async function initializeQuotaPersistence() {
    console.log('\n=== Initializing Quota Persistence ===');
    await initializeQuotaIndex();
    await loadQuotaFromOpenSearch();
    
    // Start periodic sync: every 60 seconds
    setInterval(async () => {
        await syncQuotaToOpenSearch();
    }, 60000); // 60 seconds
    
    console.log('✓ Quota persistence initialized (sync every 60s)');
    console.log('=====================================\n');
})();

// ======================================================================
// END: OPENSEARCH QUOTA PERSISTENCE
// ======================================================================
...
app.use((req, res, next) => {
    ...
    // Skip quota checking for user management endpoints (admin only)
    if (req.path.startsWith('/api/users/')) {
        return next();
    }

    // Skip quota checking for public SPARQL proxy
    if (req.path === '/api/sparql-proxy') {
        return next();
    }

    // Skip quota checking for public endpoints (no API key required)
    const publicEndpoints = ['/version', '/reset-rate-limit', '/support', '/dump-api-keys'];
    if (publicEndpoints.includes(req.path)) {
        return next();
    }

    // === EARLY API KEY VALIDATION ===
    // Reject requests with missing or invalid API keys BEFORE touching quota.
    // This prevents bots and scanners from consuming quota.
    ...
    if (!req.apiKey || !hasKey) {
        console.log(`[quota] Rejected unauthorized request: key=${req.apiKey} IP=${req.ip} path=${req.path}`);
    ...
    if (reg && reg.blocked) {
        console.log(`Blocked user access attempt: ${reg.email || req.apiKey} from IP: ${req.ip}`);
        return res.status(403).json({
            error: 'Access forbidden: Your account has been blocked. Please contact support.'
        });
    }
    
    // Check if subscription/trial has expired
    if (reg && reg.expiresAt) {
        const now = new Date();
        const expirationDate = new Date(reg.expiresAt);
        if (now > expirationDate) {
            console.log(`Expired account access attempt: ${reg.email || req.apiKey} (expired: ${reg.expiresAt})`);
            return res.status(403).json({
                error: 'Access forbidden: Your subscription has expired. Please renew to continue using the service.'
            });
        }
    }
    ...
    if (isTrialKey) {
        ...
        if (!emailData || emailData.date !== today) {
            emailData = { count: 0, date: today, clientType };
        }
        emailData.count += 1;
        emailData.clientType = clientType; // Update on each request
        ...
        if (!ipData || ipData.date !== today) {
            ipData = { count: 0, date: today, clientType };
        }
        ipData.count += 1;
        // Store email and client type in IP data for trial keys
        ipData.email = clientEmail;
        ipData.clientType = clientType;
        ...
        // Regular keys: track by email (not IP) to avoid conflicts when multiple users share an IP
        const clientType = detectClientType(req);
        
        // Get email from header or registry
        const email = clientEmail || (reg && reg.email) || 'no-email';
        
        // Use email as the key for paid users (or passkey if no email)
        const userKey = email !== 'no-email' ? `paid:${email}` : `paid-key:${req.apiKey}`;
        
        let usageData = ipRequestCounts.get(userKey);

        if (!usageData || usageData.date !== today) {
            usageData = { count: 0, date: today, clientType };
        }

        usageData.count += 1;
        // Store email and client type
        usageData.email = email;
        usageData.clientType = clientType;
        ipRequestCounts.set(userKey, usageData);
...
app.use((req, res, next) => {
    ...
    const publicEndpoints = ['/version', '/reset-rate-limit', '/support', '/api/sparql-proxy'];
    if (publicEndpoints.includes(req.path)) {
        return next();
    }
...
app.post('/search', express.json({ limit: '1mb' }), async (req, res) => {
    try {
        ...
        const timestamp = new Date().toISOString();
        console.log(`${timestamp} REQUEST: search, index=${indexParam}, pattern=${pattern || 1}, langs=[${langsStr}]${LOG_SEARCH_TERMS ? `, term="${term}"` : ''}`);
        ...
        // Track search statistics in user registry
        const apiKey = req.header('x-api-key');
        if (apiKey && API_KEY_REGISTRY.has(apiKey)) {
            const userData = API_KEY_REGISTRY.get(apiKey);
            if (hitCount > 0) {
                userData.searchesWithResults = (userData.searchesWithResults || 0) + 1;
            } else {
                userData.searchesNoResults = (userData.searchesNoResults || 0) + 1;
            }
            // Save updated statistics to file (async, don't wait)
            reloadApiKeys().then(cfg => {
                const userInFile = cfg.keys.find(k => k.key === apiKey);
                if (userInFile) {
                    userInFile.searchesWithResults = userData.searchesWithResults;
                    userInFile.searchesNoResults = userData.searchesNoResults;
                    saveApiKeys(cfg.keys).catch(err => 
                        console.error('Failed to save search statistics:', err)
                    );
                }
            }).catch(err => console.error('Failed to update search statistics:', err));
        }
...
// ------------------------------
// POST /api/sparql-proxy
// Body: { query: "SPARQL query string" }
// Proxies SPARQL requests to Europa Publications endpoint to avoid CORS issues in browser
app.post('/api/sparql-proxy', express.json({ limit: '1mb' }), async (req, res) => {
    try {
        ...
        // Make request to Europa SPARQL endpoint
        const response = await new Promise((resolve, reject) => {
            ...
            req.on('timeout', () => {
                req.destroy();
                reject(new Error('SPARQL request timeout'));
            });
        ...
        res.status(500).json({ error: 'Failed to fetch SPARQL data' });
    }
});

// ------------------------------
// Bulk Upload Endpoint (parameter-based)
// ------------------------------
// POST /upload
...
app.get('/version', (req, res) => {
    res.status(200).json({
        version: '1.0.1',  // Update this value when releasing new versions
        minVersion: '1.0.1',
...
// ------------------------------
// Check Document Existence Endpoint
// ------------------------------
// GET /check-exists/:index/:celex
// Checks if a document with the given CELEX exists in the specified index
// Returns: { exists: true/false, celex: string, index: string }
// Note: Available to all authenticated users for deduplication purposes
app.get('/check-exists/:index/:celex', async (req, res) => {
    try {
        const { index, celex } = req.params;
        const timestamp = new Date().toISOString();
        console.log(`${timestamp} REQUEST: check-exists, index=${index}, celex=${celex}, apiKey=${req.apiKey}, IP=${req.ip}`);
        ...
        // No index permission check - allow all users to check document existence
        // This is safe because it's read-only and only returns existence (not content)
        ...
        console.log(`${timestamp} SEARCH QUERY: ${searchBody}`);
        ...
        console.log(`${timestamp} OPENSEARCH URL: http://${options.host}:${options.port}${options.path}`);
        console.log(`${timestamp} REQUEST METHOD: ${options.method}`);
        ...
        const proxyReq = http.request(options, (proxyRes) => {
            ...
            proxyRes.on('end', () => {
                try {
                    ...
                    console.log(`${timestamp} OPENSEARCH RESPONSE: ${JSON.stringify(result)}`);
                    console.log(`${timestamp} RESULT: check-exists, index=${index}, celex=${celex}, exists=${exists}, hits=${hits}`);
                    ...
                    console.error('Failed to parse OpenSearch response:', err, 'Raw data:', data);
        ...
        proxyReq.on('error', (err) => {
            ...
            res.status(502).json({ error: 'Failed to reach OpenSearch' });
        });
        
        proxyReq.write(searchBody);
        proxyReq.end();
        
    } catch (err) {
        console.error('Check-exists route error:', err);
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
async function handleSearchOrBulk(req, res, targetPath) {
    ...
    const proxyReq = http.request({
        ...
        proxyRes.on('end', () => {
            ...
            if (statusCode >= 400) {
                console.error(`${ts} ❌ OpenSearch ERROR: ${requestType} returned status ${statusCode}`);
                console.error(`${ts} Path: ${targetPath}`);
                console.error(`${ts} Payload size: ${postData.length} bytes`);
                console.error(`${ts} Response: ${data.substring(0, 500)}`);
            }
            
            try {
                const parsed = JSON.parse(data);
                // Log result count for search operations
                if (isSearch && statusCode < 400) {
    ...
    proxyReq.on('timeout', () => {
        const ts = new Date().toISOString();
        console.error(`${ts} ⏱️  REQUEST TIMEOUT: ${requestType} to ${targetPath}`);
        console.error(`${ts} Payload size: ${postData.length} bytes`);
        proxyReq.destroy();
        res.status(504).json({ error: 'Gateway Timeout - Request to OpenSearch timed out' });
    });
    
    proxyReq.on('error', (err) => {
        const ts = new Date().toISOString();
        console.error(`${ts} ❌ Proxy request error:`, err);
        console.error(`${ts} Path: ${targetPath}`);
        console.error(`${ts} Payload size: ${postData.length} bytes`);
        res.status(502).json({ error: 'Bad Gateway' });
    });
...
app.post('/:index/_search', express.raw({ limit: '20mb', type: '*/*' }), async (req, res) => {
    try {
...
app.post('/:index/_bulk', express.raw({ limit: '200mb', type: '*/*' }), async (req, res) => {
    try {
...
// Delete all documents for a CELEX from an index (delete-by-query)
// SECURITY: same ownership check as index delete
app.post('/:index/_delete_by_query', express.json({ limit: '1mb' }), async (req, res) => {
    try {
        const index = req.params.index;
        ...
        if (!isAdmin && !index.startsWith(`eu_${req.apiKey}_`)) {
            console.log(`BLOCKED: User ${req.apiKey} attempted delete-by-query on index: ${index}`);
            return res.status(403).json({ error: 'You can only modify your own indices' });
        }

        const timestamp = new Date().toISOString();
        console.log(`${timestamp} REQUEST: delete-by-query, index=${index}, apikey=${req.apiKey}, IP=${req.ip}`);

        const authHeader = 'Basic ' + Buffer.from('admin:admin').toString('base64');
        const body = JSON.stringify(req.body);
        // Forward refresh and conflicts query params from the client
        const qs = req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : '';

        const dbqReq = http.request({
            host: '127.0.0.1',
            port: 9200,
            method: 'POST',
            path: `/${index}/_delete_by_query${qs}`,
            headers: {
                'Authorization': authHeader,
                'Content-Type': 'application/json',
        ...
        dbqReq.on('error', (err) => {
            console.error('Delete-by-query error:', err);
            res.status(502).json({ error: 'Bad Gateway' });
        });
        dbqReq.write(body);
        dbqReq.end();
    } catch (err) {
        console.error('Delete-by-query route error:', err);
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
app.post('/proxy/:index/_search', express.raw({ limit: '20mb', type: '*/*' }), async (req, res) => {
    try {
        const targetPath = `${req.params.index}/_search`;
        await handleSearchOrBulk(req, res, targetPath);
    } catch (err) {
...
app.post('/proxy/:index/_bulk', express.raw({ limit: '200mb', type: '*/*' }), async (req, res) => {
    try {
        const targetPath = `${req.params.index}/_bulk`;
        await handleSearchOrBulk(req, res, targetPath);
    } catch (err) {
...
// Helper function to reload API keys from file
async function reloadApiKeys() {
    try {
        ...
        if (cfg && Array.isArray(cfg.keys)) {
            API_KEY_REGISTRY = new Map(cfg.keys.map(k => [k.key, k]));
            console.log(`Reloaded ${API_KEY_REGISTRY.size} API keys`);
            return cfg;
        }
        throw new Error('Invalid api-keys.json structure');
    } catch (e) {
...
// Helper function to save API keys to file
async function saveApiKeys(keysArray) {
    try {
        const apiKeysPath = path.join(__dirname, 'api-keys.json');
        const cfg = { keys: keysArray };
        await fs.writeFile(apiKeysPath, JSON.stringify(cfg, null, 2), 'utf8');
        API_KEY_REGISTRY = new Map(keysArray.map(k => [k.key, k]));
        console.log(`Saved ${keysArray.length} API keys to file`);
    } catch (e) {
...
// Middleware to check admin access (require specific admin key)
function requireAdmin(req, res, next) {
    const adminKey = req.headers['x-admin-key'];
    // Check if admin key matches the first key in registry (7239) or a specific admin key
    const firstKey = Array.from(API_KEY_REGISTRY.keys())[0];
    if (adminKey === firstKey || adminKey === '7239') {
        next();
    } else {
...
// GET /api/users/list - List all users with their quota and statistics
app.get('/api/users/list', requireAdmin, async (req, res) => {
    try {
        const now = new Date();
        const today = new Date().toISOString().slice(0, 10);
        
        // Get historical stats from OpenSearch (last 30 days)
        ...
        try {
            ...
            if (result.statusCode === 200 && result.body.hits) {
                ...
                for (const hit of hits) {
                    const doc = hit._source;
                    const { key, email, count, date, clientType } = doc;
                    
                    // Determine user identifier
                    let userId = email || key;
                    if (key.startsWith('trial:')) {
                        userId = key.substring(6); // trial email
                    } else if (key.startsWith('trial-ip:')) {
                        continue; // Skip IP tracking
                    }
                    
                    if (!historicalStats[userId]) {
                        historicalStats[userId] = {
                            totalSearches30Days: 0,
                            activeDays: new Set(),
                            dailyCounts: {},
                            firstSeen: date,
                            lastSeen: date,
                            // Client type breakdowns
                            webSearches: 0,
                            appSearches: 0,
                            tradosSearches: 0,
                            unknownSearches: 0
                    ...
                    const stats = historicalStats[userId];
                    stats.totalSearches30Days += count;
                    stats.activeDays.add(date);
                    stats.dailyCounts[date] = (stats.dailyCounts[date] || 0) + count;
                    
                    // Aggregate by client type
                    const client = clientType || 'unknown';
                    if (client === 'web') stats.webSearches += count;
                    else if (client === 'app') stats.appSearches += count;
                    else if (client === 'trados') stats.tradosSearches += count;
                    else stats.unknownSearches += count;
                ...
                for (const userId in historicalStats) {
                    const stats = historicalStats[userId];
                    ...
                    for (let i = 6; i >= 0; i--) {
                        ...
                        last7Days.push(stats.dailyCounts[dateStr] || 0);
                    }
                    stats.last7DaysTotal = last7Days.reduce((sum, count) => sum + count, 0);
                    
                    // Days since last active
                    const lastDate = new Date(stats.lastSeen);
                    stats.daysSinceLastActive = Math.floor((Date.now() - lastDate.getTime()) / (1000 * 60 * 60 * 24));
                    
                    // Days since first use
                    const firstDate = new Date(stats.firstSeen);
                    stats.daysSinceFirstUse = Math.floor((Date.now() - firstDate.getTime()) / (1000 * 60 * 60 * 24));
                    
                    // Average searches per active day
                    stats.avgSearchesPerDay = stats.activeDaysCount > 0 
                        ? Math.round(stats.totalSearches30Days / stats.activeDaysCount) 
                        : 0;
                }
            }
        } catch (err) {
            console.error('Failed to load historical stats:', err);
            // Continue without historical stats
        }
        
        const users = Array.from(API_KEY_REGISTRY.entries()).map(([key, userData]) => {
            // Get usage stats from ipRequestCounts (today only)
            let quotaUsed = 0;
            let email = userData.email || 'no-email';
            
            // Find usage for this key/email
            if (key === 'trial') {
                // For trial, sum all trial: prefixed entries
                for (const [countKey, countData] of ipRequestCounts.entries()) {
                    if (countKey.startsWith('trial:') && countData.date === today) {
                        quotaUsed += countData.count || 0;
                    }
                }
            } else {
                // For regular keys, find by paid: prefix with matching email
                const paidKey = `paid:${email}`;
                const paidKeyAlt = `paid-key:${key}`; // Fallback for users without email
                
                for (const [countKey, countData] of ipRequestCounts.entries()) {
                    if ((countKey === paidKey || countKey === paidKeyAlt) && countData.date === today) {
                        quotaUsed += countData.count || 0;
                    }
                }
            }
            
            const quotaRemaining = Math.max(0, (userData.dailyQuota || 1000) - quotaUsed);
            const isExpired = userData.expiresAt ? now > new Date(userData.expiresAt) : false;
            
            // Get historical stats for this user
            const userHistStats = historicalStats[email] || {
                totalSearches30Days: 0,
                last7DaysTotal: 0,
                activeDaysCount: 0,
                avgSearchesPerDay: 0,
            ...
            return {
                key,
                email,
                dailyQuota: userData.dailyQuota || 1000,
                quotaUsed,
                quotaRemaining,
                allowPrefixes: userData.allowPrefixes || [],
                userType: userData.userType || 'paid',
                blocked: userData.blocked || false,
                expiresAt: userData.expiresAt || null,
                isExpired: isExpired,
                searchesWithResults: userData.searchesWithResults || 0,
                searchesNoResults: userData.searchesNoResults || 0,
                totalSearches: (userData.searchesWithResults || 0) + (userData.searchesNoResults || 0),
                // Historical stats (last 30 days)
                totalSearches30Days: userHistStats.totalSearches30Days,
                last7DaysTotal: userHistStats.last7DaysTotal,
                activeDaysCount: userHistStats.activeDaysCount,
                avgSearchesPerDay: userHistStats.avgSearchesPerDay,
                daysSinceLastActive: userHistStats.daysSinceLastActive,
                daysSinceFirstUse: userHistStats.daysSinceFirstUse,
                firstSeen: userHistStats.firstSeen,
                lastSeen: userHistStats.lastSeen,
                // Client type breakdown
                webSearches: userHistStats.webSearches,
                appSearches: userHistStats.appSearches,
                tradosSearches: userHistStats.tradosSearches,
                unknownSearches: userHistStats.unknownSearches
            };
        });
...
app.post('/api/users/add', requireAdmin, express.json(), async (req, res) => {
    try {
        const { email, passkey, dailyQuota, userType, allowPrefixes, expiresAt } = req.body;
        ...
        if (API_KEY_REGISTRY.has(passkey)) {
            return res.status(400).json({ error: 'Passkey already exists' });
        }
        
        // Load current keys
        const cfg = await reloadApiKeys();
        
        // Add new user
        const newUser = {
            key: passkey,
            email,
            dailyQuota: dailyQuota || 1000,
            userType: userType || 'paid',
            allowPrefixes: allowPrefixes || [`eu_${passkey}_`],
            blocked: false,
        ...
        cfg.keys.push(newUser);
        await saveApiKeys(cfg.keys);
...
// DELETE /api/users/delete - Delete a user (requires email + passkey confirmation)
app.delete('/api/users/delete', requireAdmin, express.json(), async (req, res) => {
    try {
        const { email, passkey } = req.body;
        
        if (!email || !passkey) {
            return res.status(400).json({ error: 'Both email and passkey are required for deletion' });
        }
        
        const userData = API_KEY_REGISTRY.get(passkey);
        if (!userData) {
            return res.status(404).json({ error: 'User not found' });
        }
        
        if (userData.email !== email) {
            return res.status(400).json({ error: 'Email does not match passkey' });
        }
        
        // Load current keys and filter out the one to delete
        const cfg = await reloadApiKeys();
        cfg.keys = cfg.keys.filter(k => k.key !== passkey);
        await saveApiKeys(cfg.keys);
...
// POST /api/users/set-quota - Update user quota
app.post('/api/users/set-quota', requireAdmin, express.json(), async (req, res) => {
    try {
        const { passkey, dailyQuota } = req.body;
        
        if (!passkey || typeof dailyQuota !== 'number' || dailyQuota < 0) {
            return res.status(400).json({ error: 'Valid passkey and dailyQuota are required' });
        }
        
        const cfg = await reloadApiKeys();
        const user = cfg.keys.find(k => k.key === passkey);
        ...
        res.json({ success: true, message: 'Quota updated successfully', user });
    } catch (err) {
...
app.post('/api/users/block', requireAdmin, express.json(), async (req, res) => {
    try {
        const { passkey, blocked } = req.body;
        
        if (!passkey || typeof blocked !== 'boolean') {
            return res.status(400).json({ error: 'Passkey and blocked status are required' });
        }
        ...
        res.status(500).json({ error: 'Failed to block/unblock user', details: err.message });
    }
});

// POST /api/users/set-type - Change user type (paid, trial, grace)
app.post('/api/users/set-type', requireAdmin, express.json(), async (req, res) => {
    try {
        const { passkey, userType } = req.body;
        
        if (!passkey || !userType) {
            return res.status(400).json({ error: 'Passkey and userType are required' });
        }
        
        const validTypes = ['paid', 'trial', 'grace'];
        if (!validTypes.includes(userType)) {
            return res.status(400).json({ error: 'Invalid user type. Must be: paid, trial, or grace' });
        }
        ...
        res.json({ 
            success: true, 
            message: `User type changed to ${userType} successfully`, 
            user 
        });
    } catch (err) {
        console.error('Failed to change user type:', err);
        res.status(500).json({ error: 'Failed to change user type', details: err.message });
    }
});

// POST /api/users/reset-daily-usage - Reset daily usage for a specific user
app.post('/api/users/reset-daily-usage', requireAdmin, express.json(), async (req, res) => {
    try {
        ...
        // Reset daily usage by removing relevant entries from ipRequestCounts
        const email = user.email || 'no-email';
        const today = new Date().toISOString().slice(0, 10);
        
        // Find and reset all entries for this user's email today
        let resetCount = 0;
        for (const [key, data] of ipRequestCounts.entries()) {
            if (data.date === today && data.email === email) {
                ipRequestCounts.delete(key);
                resetCount++;
            }
        }
        
        // Also handle trial key entries (trial:email format)
        const trialEmailKey = `trial:${email.toLowerCase().trim()}`;
        if (ipRequestCounts.has(trialEmailKey)) {
            ipRequestCounts.delete(trialEmailKey);
            resetCount++;
        }
        
        console.log(`Reset daily usage for ${email}, removed ${resetCount} entries`);
        
        res.json({ 
            success: true, 
            message: `Daily usage reset successfully for ${email}`,
            resetCount
        });
    } catch (err) {
        console.error('Failed to reset daily usage:', err);
        res.status(500).json({ error: 'Failed to reset daily usage', details: err.message });
    }
});

// GET /api/users/trial-emails - Get list of all trial emails with timestamps
app.get('/api/users/trial-emails', requireAdmin, async (req, res) => {
    try {
        const emails = [];
        try {
            const content = await fs.readFile(emailsFilePath, 'utf8');
        ...
        res.json({ 
            ...
            uniqueCount: new Set(emails.map(e => e.email)).size
        });
    } catch (err) {
        console.error('Failed to retrieve trial emails:', err);
        res.status(500).json({ error: 'Failed to retrieve trial emails', details: err.message });
    }
});

// GET /api/users/statistics - Get aggregated statistics for all users
app.get('/api/users/statistics', requireAdmin, (req, res) => {
    try {
        const today = new Date().toISOString().slice(0, 10);
        const now = new Date();
        const stats = {
            totalUsers: API_KEY_REGISTRY.size,
            activeToday: 0,
        ...
        for (const [key, userData] of API_KEY_REGISTRY.entries()) {
            stats.totalQuotaAllocated += (userData.dailyQuota || 1000);
            
            // Count expired accounts
            if (userData.expiresAt && now > new Date(userData.expiresAt)) {
            ...
            if (key === 'trial') {
                for (const [countKey, countData] of ipRequestCounts.entries()) {
                    if (countKey.startsWith('trial:') && countData.date === today) {
                        userQuotaUsed += countData.count || 0;
                        // Aggregate client types
                        const clientType = countData.clientType || 'unknown';
                        if (clientType === 'web') stats.webSearchesToday += countData.count || 0;
                        else if (clientType === 'app') stats.appSearchesToday += countData.count || 0;
                        else if (clientType === 'trados') stats.tradosSearchesToday += countData.count || 0;
                        else stats.unknownSearchesToday += countData.count || 0;
                    }
                }
            } else {
                // For paid users, look for paid: prefix entries
                const paidKey = `paid:${email}`;
                const paidKeyAlt = `paid-key:${key}`;
            ...
            if (userQuotaUsed > 0) {
                stats.activeToday++;
                stats.totalQuotaUsed += userQuotaUsed;
                stats.totalSearchesToday += userQuotaUsed;
            }
        }
...
// GET /api/users/historical-stats - Get historical usage statistics from OpenSearch
app.get('/api/users/historical-stats', requireAdmin, async (req, res) => {
    try {
        ...
        for (const hit of hits) {
            const doc = hit._source;
            const { key, email, count, date } = doc;
            ...
            if (key.startsWith('trial:')) {
                userKey = 'trial';
                userEmail = key.substring(6); // Remove 'trial:' prefix
            } else if (key.startsWith('trial-ip:')) {
                continue; // Skip IP tracking, we already have email tracking
            } else {
                // Regular user - find by email in registry
                for (const [regKey, regData] of API_KEY_REGISTRY.entries()) {
                    if (regData.email === email) {
                        userKey = regKey;
                        break;
                    }
                }
                if (!userKey) userKey = key; // Fallback to IP as key
            }
        ...
        for (const userKey in userStats) {
            ...
            // Calculate average searches per active day
            stats.avgSearchesPerDay = stats.activeDays > 0 
                ? Math.round(stats.totalSearches / stats.activeDays) 
                : 0;
            
            // Calculate days since first use
            const firstDate = new Date(stats.firstSeen);
            const daysSinceFirst = Math.floor((Date.now() - firstDate.getTime()) / (1000 * 60 * 60 * 24));
            stats.daysSinceFirstUse = daysSinceFirst;
            
            // Calculate days since last use
            const lastDate = new Date(stats.lastSeen);
            const daysSinceLast = Math.floor((Date.now() - lastDate.getTime()) / (1000 * 60 * 60 * 24));
            stats.daysSinceLastActive = daysSinceLast;
            ...
            stats.last7DaysTrend = last7Days;
            stats.last7DaysTotal = last7Days.reduce((sum, count) => sum + count, 0);
        }
        
        res.json({ success: true, stats: userStats, period: { start: startDateStr, end: today, days } });
    } catch (err) {
        console.error('Failed to get historical stats:', err);
        res.status(500).json({ error: 'Failed to get historical stats', details: err.message });
    }
});
