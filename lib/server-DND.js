...
const nodemailer = require('nodemailer'); // For email forwarding
...
async function trackUniqueEmail(email) {
    ...
    try {
        await fs.appendFile(emailsFilePath, `${normalizedEmail}\n`);
    } catch (err) {
...
app.get('/version', (req, res) => {
    res.status(200).json({
        version: '1.0.0',  // Update this value when releasing new versions
        minVersion: '1.0.0',
        updateUrl: 'https://www.pts-translation.sk/download'
    });
});
...
app.get('/_cat/indices', async (req, res) => {
    try {
        ...
        const reg = API_KEY_REGISTRY.get(req.apiKey);
        const isAdmin = reg && reg.email && reg.email.toLowerCase().includes('admin');
...
app.post('/:index/_bulk', express.raw({ limit: '10mb', type: '*/*' }), async (req, res) => {
    try {
...
app.delete('/:index', async (req, res) => {
    try {
        ...
        // Comment this check if you want to allow deletion of any index
        if (!index.startsWith(`eu_${req.apiKey}_`)) {
...
// ------------------------------
// Proxy route (alternative with /proxy prefix)
// ------------------------------
// POST /proxy/:index/_search or /proxy/:index/_bulk
app.post('/proxy/*', express.raw({ limit: '10mb', type: '*/*' }), async (req, res) => {
    try {
